...
// +----+----------+------+----------+
    public static List<User> getUsersByPageOrderedByIdDesc(Connection databaseConnection, int pageNum, int pageSize) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select * from USER ORDER by ID desc limit ? OFFSET ?")) {
            statement.setInt(1, pageSize * (pageNum - 1) + 1);
...
//  +----+--------+------+
    public static List<GoodsAndGmv> getGoodsAndGmv(Connection databaseConnection) throws SQLException {
        try(PreparedStatement sql = databaseConnection.prepareStatement("select goods.ID,goods.name,sum(GOODS_NUM * GOODS_PRICE) as GMV " +
                "from \"ORDER\",GOODS " +
                "where GOODS_ID = goods.ID " +
                "group by name order by GMV desc")) {
            ResultSet resultSet = sql.executeQuery();
            ArrayList<GoodsAndGmv> result = new ArrayList<>();
            while (resultSet.next()) {
...
// +----------+-----------+------------+-------------+
    public static List<Order> getInnerJoinOrders(Connection databaseConnection) throws SQLException {
        try(PreparedStatement sql = databaseConnection.prepareStatement("select \"ORDER\".ID as ORDER_ID," +
                "USER.NAME AS USER_NAME," +
                "GOODS.NAME AS GOODS_NAME," +
                "GOODS_NUM * GOODS_PRICE AS TOTAL_PRICE " +
                "from goods,user,\"ORDER\"" +
