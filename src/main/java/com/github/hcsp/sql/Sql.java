...
//  +----+--------+------+
    static List<GoodsAndGmv> getGoodsAndGmv(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select `order`.goods_id,goods.name,SUM((`order`.goods_num * `order`.goods_price)) AS TOTAL_MONEY from `order`\n" +
                "join goods\n" +
