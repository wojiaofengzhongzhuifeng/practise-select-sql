...
// +------------+-----------------+------------------+---------------------+-------------------------------+
     ...
     * @param databaseConnection 数据库连接对象
     * @return 有多少用户买过这个商品
...
//  +----+--------+------+
    public static List<GoodsAndGmv> getGoodsAndGmv(Connection databaseConnection) throws SQLException {
        ...
        try (PreparedStatement statement = databaseConnection.prepareStatement("select \"GOODS\".id,GOODS.name, sum(\"ORDER\".GOODS_NUM*\"ORDER\".GOODS_PRICE) as gmv from \"ORDER\"\n" +
...
// +----------+-----------+------------+-------------+
    public static List<Order> getInnerJoinOrders(Connection databaseConnection) throws SQLException {
        ...
        try (PreparedStatement statement = databaseConnection.prepareStatement("select \"ORDER\".id, USER.NAME, GOODS.NAME,\"ORDER\".GOODS_NUM*\"ORDER\".GOODS_PRICE as TOTAL_PRICE\n" +
            ...
            statement.executeQuery();
            ResultSet resultSet = statement.getResultSet();
            conversionResultSetToOrderList(resultSet, orderList);
        }
        return orderList;
    }
...
// +----------+-----------+------------+-------------+
    ...
    private static void conversionResultSetToOrderList(ResultSet resultSet, List<Order> orderList) throws SQLException {
        while (resultSet.next()) {
