...
//  +----+--------+------+
    public static List<GoodsAndGmv> getGoodsAndGmv(Connection databaseConnection) throws SQLException {
        ...
        try (PreparedStatement statement = databaseConnection.prepareStatement(
                "select \"ORDER\".GOODS_ID, NAME, SUM(GOODS_NUM * GOODS_PRICE) AS GMV\n" +
                        "FROM \"ORDER\"\n" +
                        "         join GOODS ON \"ORDER\".GOODS_ID = GOODS.ID\n" +
                        "group by Name, \"ORDER\".GOODS_ID\n" +
                        "ORDER BY GMV DESC")) {
            ResultSet resultSet = statement.executeQuery();
            while (resultSet.next()) {
...
// +----------+-----------+------------+-------------+
    public static List<Order> getInnerJoinOrders(Connection databaseConnection) throws SQLException {
        return getOrders(databaseConnection, "select \"ORDER\".ID AS ORDER_ID, USER.NAME AS  USER_NAME, GOODS.NAME AS GOODS_NAME, GOODS_NUM * GOODS_PRICE AS TOTAL_PRICE\n" +
                "FROM \"ORDER\"\n" +
                "         INNER JOIN GOODS\n" +
