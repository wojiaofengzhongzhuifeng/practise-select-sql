...
//  +----+--------+------+
    public static List<GoodsAndGmv> getGoodsAndGmv(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select GOODS.ID, NAME, (GOODS_PRICE * sum(GOODS_NUM)) as GMV\n" +
                "from GOODS,\n" +
                "     \"ORDER\"\n" +
...
// +----------+-----------+------------+-------------+
    public static List<Order> getInnerJoinOrders(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select table_order.ID                                  as order_id,\n" +
                "       table_user.NAME                                 as user_name,\n" +
                "       table_goods.NAME                                as good_name,\n" +
                "       table_order.GOODS_PRICE * table_order.GOODS_NUM as TOTAL_PRICE\n" +
                "from (\"ORDER\" table_order\n" +
                "    inner join GOODS table_goods on table_goods.ID = table_order.GOODS_ID)\n" +
                "         inner join USER table_user on table_user.id = table_order.USER_ID\n" +
                "where table_user.NAME != ''\n" +
                "  and table_goods.name != ''")) {
            ResultSet resultSet = statement.executeQuery();
...
// +----------+-----------+------------+-------------+
    public static List<Order> getLeftJoinOrders(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select \"ORDER\".id, user.NAME, GOODS.name as good_name,\"ORDER\".GOODS_NUM*\"ORDER\".GOODS_PRICE\n" +
                "from \"ORDER\"\n" +
