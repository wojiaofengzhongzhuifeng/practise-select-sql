...
// +------------+-----------------+------------------+---------------------+-------------------------------+
    ...
    public static List<User> getUsersByPageOrderedByIdDesc(Connection databaseConnection, int pageNum, int pageSize) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("SELECT * FROM USER ORDER BY ID DESC LIMIT ? OFFSET ?;")) {
            int beginNum = pageNum * (pageSize - 1);
            statement.setInt(1, beginNum);
...
// +----------+-----------+------------+-------------+
    public static List<Order> getInnerJoinOrders(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("SELECT \"ORDER\".ID AS order_id, User.name as user_name, GOODS.name AS goods_name, GOODS_PRICE * GOODS_NUM FROM \"ORDER\"\n" +
