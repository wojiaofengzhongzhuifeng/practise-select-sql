...
// +----+----------+------+----------+
    public static List<User> getUsersByPageOrderedByIdDesc(Connection databaseConnection, int pageNum, int pageSize) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("SELECT * FROM user ORDER BY id DESC LIMIT ?,?")) {
            statement.setInt(1, (pageNum - 1) * pageSize);  //此处注意
...
// +----------+-----------+------------+-------------+
    public static List<Order> getLeftJoinOrders(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("SELECT `order`.id AS ORDER_ID,USER.NAME AS USER_NAME,GOODS.NAME " +
                "AS GOODS_NAME ,GOODS_NUM*GOODS_PRICE AS TOTAL_PRICE FROM `ORDER`\n" +
                "LEFT JOIN GOODS\n" +
                "ON `ORDER`.GOODS_ID =GOODS.ID\n" +
                "LEFT JOIN USER\n" +
                "ON `ORDER`.USER_ID = USER.ID")) {
