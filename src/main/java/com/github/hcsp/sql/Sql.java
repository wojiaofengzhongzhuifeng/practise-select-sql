...
// +------------+-----------------+------------------+---------------------+-------------------------------+
     ...
     * @return 有多少用户买过这个商品
     * @throws SQLException sql语法错误或者没有相对应的结果列
     */
...
//  +----+--------+------+
    public static List<GoodsAndGmv> getGoodsAndGmv(Connection databaseConnection) throws SQLException {
        String sql = "select `ORDER`.GOODS_ID AS ID, GOODS.NAME AS NAME, SUM(GOODS_NUM * GOODS_PRICE) AS GMV  from `ORDER` \n" + "" +
     ...
     *
     * @param databaseConnection 数据库连接
     * @return 用户id、姓名、商品名和订单价格总和的list
     * @throws SQLException sql语法错误或者没有相对应的结果列
     */
// 预期的结果为：
// +----------+-----------+------------+-------------+
...
// +----------+-----------+------------+-------------+
    public static List<Order> getInnerJoinOrders(Connection databaseConnection) throws SQLException {
        String sql = "select `ORDER`.ID, USER.NAME AS USER_NAME, GOODS.NAME AS GOODS_NAME, GOODS_NUM * GOODS_PRICE AS TOTAL_PRICE from `ORDER` \n" +
     ...
     *
     * @param databaseConnection 数据库连接
     * @return 用户id、姓名、商品名和订单价格总和的list（包含没下过单的商品）
     * @throws SQLException sql语法错误或者没有相对应的结果列
     */
// 预期的结果为：
// +----------+-----------+------------+-------------+
...
// +----------+-----------+------------+-------------+
    public static List<Order> getLeftJoinOrders(Connection databaseConnection) throws SQLException {
        String sql = "select `ORDER`.ID ,user.NAME ,GOODS.NAME , PRICE*GOODS_NUM as totalPrice from \"ORDER\"\n" +
                "left join GOODS on \"ORDER\".GOODS_ID=goods.ID\n" +
                "left join USER on \"ORDER\".ID=USER.ID";
        List<Order> orders = new ArrayList<>();
        try (PreparedStatement preparedStatement = databaseConnection.prepareStatement(sql)) {
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
