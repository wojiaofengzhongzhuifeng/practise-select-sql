...
//  +----+--------+------+
    public static List<GoodsAndGmv> getGoodsAndGmv(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select GOODS_ID as id, name, sum(GOODS_PRICE * GOODS_NUM) as gmv from \"ORDER\" join GOODS on \"ORDER\".GOODS_ID = goods.id group by goods_id order by gmv desc ")) {
            ResultSet resultSet = statement.executeQuery();
...
// +----------+-----------+------------+-------------+
    public static List<Order> getInnerJoinOrders(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement(
                "SELECT \"ORDER\".ID, USER.NAME, \"GOODS\".NAME, GOODS_NUM * GOODS.PRICE AS TOTAL_PRICE FROM \"ORDER\" INNER JOIN USER ON USER.ID = \"ORDER\".USER_ID INNER JOIN GOODS ON GOODS.ID = \"ORDER\".GOODS_ID")) {
            ResultSet resultSet = statement.executeQuery();
...
// +----------+-----------+------------+-------------+
    public static List<Order> getLeftJoinOrders(Connection databaseConnection) throws SQLException {
        try (PreparedStatement preparedStatement = databaseConnection.prepareStatement("SELECT \"ORDER\".ID,USER.NAME,GOODS.NAME,GOODS_NUM*GOODS_PRICE AS TOTAL_PRICE FROM \"ORDER\" LEFT JOIN GOODS ON GOODS.ID=\"ORDER\".GOODS_ID LEFT JOIN USER ON USER.ID=\"ORDER\".USER_ID")) {
            List<Order> list = new ArrayList<>();
